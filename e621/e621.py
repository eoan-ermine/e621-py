...
class Posts:
    ...
    def create(
        ...
        return self._api.request(
            ...
            upload={
                ...
                "md5_confirmation": md5_confirmation,
                "as_pending": as_pending,
            },
            type="POST",
        )
    ...
    def update(
        ...
        is_note_locked=None,
        edit_reason=None,
        has_embedded_notes=None,
    ):
        return self._api.request(
            f"posts/{post_id}",
            post={
                "tag_string_diff": tag_string_diff,
                ...
                "has_embedded_notes": has_embedded_notes,
            },
            type="PATCH",
        )

    def list(self, limit: int, tags: str, page: int):
        return self._api.request("posts", limit=limit, tags=tags, page=page)

    def vote(self, post_id, score, no_unvote=None):
        return self._api.request(f"posts/{post_id}/votes", score=score, no_unvote=no_unvote, type="POST")
...
class Notes:
    ...
    def update(self, note_id, x, y, width, height, body):
        return self._api.request(
            f"notes/{note_id}",
            note={"x": x, "y": y, "width": width, "height": height, "body": body},
            type="PUT",
        )

    def delete(self, note_id):
        return self._api.request(f"notes/{note_id}", type="DELETE")

    def revert(self, note_id, version_id):
        return self._api.request(f"notes/{note_id}/", version_id=version_id, type="PUT")
...
class Pools:
    ...
    def list(
        ...
        if not all([id, description_matches, creator_name, creator_id]):
            raise RuntimeError(
                "One of arguments should be passed: id, description_matches, creator_name," " creator_id"
            )
    ...
    def revert(self, pool_id, version_id):
        return self._api.request(f"pools/{pool_id}", version_id=version_id, type="PUT")
...
class e621:
    def __init__(self, login: str, api_key: str):
        ...
        self._credentials = {"login": login, "api_key": api_key}
    ...
    def request(self, method: str, type="GET", **kwargs):
        ...
        if type == "GET":
            return requests.get(url, params=kwargs, headers=HEADERS).json()
        elif type == "POST":
            return requests.post(url, params=kwargs, headers=HEADERS).json()
        elif type == "PATCH":
            return requests.patch(url, params=kwargs, headers=HEADERS).json()
        elif type == "PUT":
            return requests.put(url, params=kwargs, headers=HEADERS)
